import pyautogui #biblioteca para identificar ações na tela
...
captura =(360, 320, altura, largura) #região da captura da tela
...
def identifica_vermelho(imagem):
    ...
    for x in range(0, altura_imagem): #identificar primeira coordenada do pixel
        for y in range(0, largura_imagem): #identificar segunda coordenada do pixel
            if imagem.getpixel((x, y)) == (255, 0, 0): #se o pixel tiver coordenada (x,y), então ele é vermelho
                return x, y #devolve coordenada (x,y)
    
while not keyboard.is_pressed('m'): #enquanto o m for pressionado, o programa é executado
